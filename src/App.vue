<template>
  <div class="layout">
    <header class="topbar">
      ...
      <nav class="tabs">
        <button v-for="t in tabs" :key="t.key" :class="{ active: tab === t.key }" @click="tab = t.key">
          ...
          <i v-else-if="t.key === 'recon' && store.pendingDiffCount" class="tab-badge recon">{{ store.pendingDiffCount }}</i>
        </button>
      </nav>
      <div class="user">
    ...
    <main class="content">
      ...
      <ReconCenter v-else-if="tab === 'recon'" />
...
import ReconCenter from '@/components/ReconCenter.vue'
...
.tab-badge.recon { background: #009688; box-shadow: 0 2px 6px rgba(0,150,136,0.5); }
